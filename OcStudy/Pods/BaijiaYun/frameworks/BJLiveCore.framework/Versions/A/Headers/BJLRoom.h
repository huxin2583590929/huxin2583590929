...
/**
 ...
 2.1. 检查网络是否可用
 2.2. 获取教室、用户及教室配置等信息，成功后 `roomInfo` 和 `loginUser` 变为可用
 2.3. 建立服务器连接
 2.4. 进入教室成功、loading 状态结束，`loadingVM` 变为 nil、调用 `enterRoomSuccess`
...
/**
 ...
 #discussion 教室类型为专业版小班课的教室，默认只支持 64-ibit 设备进入教室，32-bit 设备进教室时通过 `enterRoomFailureWithError:` 返回错误码 `BJLErrorCode_enterRoom_unsupportedDevice`
...
/**
 ...
 #discussion 需要在进教室之前设置，专属域名从百家云账号中心查看
 */
...
/**
 设置用户自定义参数，参数会作为进教室接口的入参
 这个参数必须在进教室之前设置，建议当实例化room对象后马上设置。
...
- (void)enter;
/** 进入教室
 #param validateConflict 传 YES 检查是否有相同用户在教室、如果有则回调错误 `BJLErrorCode_enterRoom_loginConflict`，传 NO 直接进入教室、相同用户将被踢出
...
/** 教室内作业管理、显示、控制，参考 `BJLHomeworkVM` */
@property (nonatomic, readonly, nullable) BJLHomeworkVM *homeworkVM;

/** 教室内云盘文件管理、显示、控制，参考 `BJLCloudDiskVM` */
